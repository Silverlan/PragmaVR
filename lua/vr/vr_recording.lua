...
local function setup_uuids()
	...
	if(primaryController ~= nil) then primaryController:GetEntity():SetUuid(util.generate_uuid_v4("vr_primary_controller")) end
	
	local secondaryController = hmdC:GetSecondaryController()
	if(secondaryController ~= nil) then secondaryController:GetEntity():SetUuid(util.generate_uuid_v4("vr_secondary_controller")) end
	...
	if(cam ~= nil) then hmdC:GetEntity():SetUuid(util.generate_uuid_v4("vr_camera")) end
end
...
vr_start_recording = function(pl,name)
	...
	local primaryController = hmdC:GetPrimaryController()
	if(primaryController ~= nil) then recorderC:AddEntity(primaryController:GetEntity()) end
	
	local secondaryController = hmdC:GetSecondaryController()
	if(secondaryController ~= nil) then recorderC:AddEntity(secondaryController:GetEntity()) end
	
	local cam = hmdC:GetCamera()
	if(cam ~= nil) then recorderC:AddEntity(cam:GetEntity()) end
...
vr_end_recording = function(cancel)
	...
	if(cancel ~= true) then
		...
		if(res == false) then console.print_warning("Unable to save recording as '" .. path .. "'")
		else print("Recording successfully saved as '" .. path .. "'!") end
	end
...
vr_render_recording = function(pl,recordingName)
	...
	else console.print_warning("Failed to load recording '" .. path .. "'...") end
end
