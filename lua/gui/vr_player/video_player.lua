	...
	if(type == "equirectangular") then
		...
		if(mode == "stereo") then
			...
			if(stereoAlignment == "vertical") then
				...
				renderFlags = bit.bor(renderFlags,shader.VREquirectangular.RENDER_FLAG_EQUIRECTANGULAR_STEREO_HORIZONTAL_BIT)
			end
		end
...
function gui.VRVideoPlayer.apply_video_settings(player,videoSettings)
	...
	if(vp ~= nil) then
		if(videoSettings.fileName ~= nil) then vp:LoadFile(videoSettings.fileName)
		elseif(videoSettings.url ~= nil) then vp:LoadURL(videoSettings.url) end
	end
	if(videoSettings.horizontalRange ~= nil) then player:SetHorizontalRange(videoSettings.horizontalRange) end
...
function gui.VRVideoPlayer:GetTexture() return self.m_videoPlayer:GetTexture() end
function gui.VRVideoPlayer:GetVideoPlayer() return self.m_videoPlayer end
function gui.VRVideoPlayer:OnRemove()
	if(self.m_videoPlayer ~= nil) then self.m_videoPlayer:Close() end
end
...
function gui.VRVideoPlayer:InitializeTexture(vpTex)
	...
	w = math.round(w)
	h = math.round(h)
	if(self.m_texture ~= nil and w == self.m_texture:GetWidth() and h == self.m_texture:GetHeight()) then return end
...
function gui.VRVideoPlayer:LinkToControls(aspectRatio,timeline,playControls)
	...
	if(playControls ~= nil) then
		playControls:AddCallback("OnButtonPressed",function(el,button)
			...
			if(button == gui.PlaybackControls.BUTTON_PLAY) then vp:Play()
			elseif(button == gui.PlaybackControls.BUTTON_PAUSE) then vp:Pause() end
		end)
		...
		vp:AddCallback("OnStateChanged",function()
			...
			if(vp:IsPlaying()) then playButton:Play()
			else playButton:Pause() end
