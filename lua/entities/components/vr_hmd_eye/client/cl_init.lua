...
function ents.VRHMDEye:SetClearColor(clearColor) self.m_clearColor = clearColor end
...
function ents.VRHMDEye:OnEntitySpawn()
	...
	if(prosper.get_api_abbreviation() == "GL") then drawSceneInfo.flags = bit.bor(drawSceneInfo.flags,game.DrawSceneInfo.FLAG_FLIP_VERTICALLY_BIT) end
...
function ents.VRHMDEye:GetHMD() return self.m_hmdC end
...
function ents.VRHMDEye:SetRenderEnabled(enabled) self.m_renderEnabled = enabled end
function ents.VRHMDEye:IsRenderEnabled() return self.m_renderEnabled end
...
function ents.VRHMDEye:DrawScene(mainDrawSceneInfo)
	...
	if(self.m_renderer == nil or camC == nil or trC == nil or self:IsRenderEnabled() == false) then return end
	if(not trC:IsUserInteractionActive() and self:GetEyeIndex() == openvr.EYE_RIGHT and not cvRenderBothEyesIfHmdInactive:GetBool()) then return end -- If HMD is not put on, we only render the left eye to save resources
	if(self:GetEyeIndex() == cvMirrorEyeView:GetInt()) then self.m_drawSceneInfo.outputImage = mainDrawSceneInfo.outputImage
	else self.m_drawSceneInfo.outputImage = nil end

	camC:UpdateViewMatrix()
	self:UpdateProjectionMatrix()
	if(util.is_valid(self.m_renderer)) then game.queue_scene_for_rendering(self.m_drawSceneInfo) end
end
